...
if False:
    L2NetKerasFeature2D = import_from(
        "feature_l2net_keras", "L2NetKerasFeature2D"
...
def feature_manager_factory(
    num_features=kNumFeatureDefault,
    ...
    return FeatureManager(
        num_features, num_levels, scale_factor, detector_type, descriptor_type
    )
...
class FeatureManager(object):
    def __init__(
        ...
        self.first_level = (
            0  # not always applicable = > 0: start pyramid from input image;
        )
        ...
        self.descriptor_distance = (
            None  # pointer to a function for computing the distance between two points
        )
        ...
        self.force_multiscale_detect_and_compute = (
            False  # automatically managed below depending on features
        )

        self.oriented_features = (
            True  # automatically managed below depending on selected features
        )
        self.do_keypoints_size_rescaling = (
            False  # automatically managed below depending on selected features
        )
        self.need_color_image = (
            False  # automatically managed below depending on selected features
        )

        self.keypoint_filter_type = (
            KeyPointFilterTypes.SAT
        ...
        if opencv_major == 3:
            ...
            MSD_create = import_from(
                "cv2", "xfeatures2d_MSDDetector"
            ...
            HL_create = import_from(
                "cv2", "xfeatures2d_HarrisLaplaceFeatureDetector", "create"
            )
        ...
        self.is_detector_equal_to_descriptor = (
            self.detector_type.name == self.descriptor_type.name
        )
        ...
        if self.descriptor_type in [
            ...
            self.scale_factor = (
                2  # the above descriptors work on octave layers with a scale_factor=2!
            )
            Printer.orange(
                "forcing scale factor=2 for detector", self.descriptor_type.name
            )
        ...
        )  # scoreType=cv2.ORB_HARRIS_SCORE, scoreType=cv2.ORB_FAST_SCORE

        # --------------------------------------------- #
        # init detector
        # --------------------------------------------- #
        if (
            self.detector_type == FeatureDetectorTypes.SIFT
            or self.detector_type == FeatureDetectorTypes.ROOT_SIFT
        ):
            ...
            self._feature_detector = self.KAZE_create(
                nOctaves=self.num_levels, threshold=0.0005
            )  # default: threshold = 0.001f
            self.scale_factor = 2  # force scale factor = 2 between octaves
            #
            #
            ...
            if self.descriptor_type != FeatureDescriptorTypes.NONE:
                ...
                self.pyramid_do_parallel = (
                    False  # N.B.: SUPERPOINT interface class is not thread-safe!
                )
            ...
            if self.descriptor_type != FeatureDescriptorTypes.NONE:
                ...
                self.use_pyramid_adaptor = (
                    self.num_levels > 1
                )  # override a pyramid adaptor?
                # self.pyramid_type = PyramidType.GAUSS_PYRAMID
                # self.first_level = 0
            ...
            self.oriented_features = False
            self._feature_detector = self.STAR_create(
                maxSize=45,
            ...
            self.oriented_features = False
            self._feature_detector = self.HL_create(
                numOctaves=self.num_levels,
        ...
        if self.is_detector_equal_to_descriptor:
            ...
            if (
                self.descriptor_type == FeatureDescriptorTypes.SIFT
                or self.descriptor_type == FeatureDescriptorTypes.ROOT_SIFT
            ):
                ...
                self._feature_descriptor = (
                    self._feature_detector
                ...
                if self.detector_type in [
                    FeatureDetectorTypes.KAZE,
                    FeatureDetectorTypes.SURF,
                ]:
                    ...
                    FeatureDetectorTypes.AKAZE,
                    FeatureDetectorTypes.AGAST,
                    FeatureDetectorTypes.FAST,
                    FeatureDetectorTypes.BRISK,
                ]:
                ...
                self._feature_descriptor = self.LUCID_create(
                    lucid_kernel=1, blur_kernel=3  # =1
                )  # =2
                self.need_color_image = True
                #
                #
        ...
        try:
            ...
            Printer.red(
                "You did not set the norm type for: ", self.descriptor_type.name
            )
            raise ValueError(
                "Unmanaged norm type for feature descriptor %s"
        ...
        try:
            ...
            Printer.red(
                "You did not set the reference max descriptor distance for: ",
                self.descriptor_type.name,
            )
            raise ValueError(
                "Unmanaged max descriptor distance for feature descriptor %s"
                % self.descriptor_type.name
            )
        Parameters.kMaxDescriptorDistanceSearchEpipolar = (
        ...
        if not self.oriented_features:
            Printer.orange(
                "WARNING: using NON-ORIENTED features: ",
                self.detector_type.name,
                "-",
                self.descriptor_type.name,
                " (i.e. kp.angle=0)",
            )
        ...
        if self.use_pyramid_adaptor:
            self.pyramid_params = dict(
                detector=self._feature_detector,
    ...
    def init_sigma_levels_sift(self):
        ...
        self.scale_factor = math.pow(2, 1.0 / 3)
        self.inv_scale_factor = 1.0 / self.scale_factor
        self.scale_factors = np.zeros(num_levels)
        self.level_sigmas2 = np.zeros(num_levels)
        self.level_sigmas = np.zeros(num_levels)
        ...
        self.level_sigmas2[
            0
        ] = sigma_level02  # -4*sift_init_sigma2  N.B.: this is an absolute sigma,
        ...
        for i in range(1, num_levels):
            self.scale_factors[i] = self.scale_factors[i - 1] * self.scale_factor
            self.level_sigmas2[i] = (
                self.scale_factors[i] * self.scale_factors[i] * sigma_level02
    ...
    def rescale_keypoint_size(self, kps):
        ...
        if self.detector_type == FeatureDetectorTypes.FAST:
            ...
            self.detector_type == FeatureDetectorTypes.SHI_TOMASI
            or self.detector_type == FeatureDetectorTypes.GFTT
        ):
    ...
    def detect(self, frame, mask=None, filter=True):
        if (
            not self.need_color_image and frame.ndim > 2
        ...
        filter_name = "NONE"
        if filter:
            kps, _, filter_name = self.filter_keypoints(
                self.keypoint_filter_type, frame, kps
            )
    ...
    def detectAndCompute(self, frame, mask=None, filter=True):
        ...
        if self.use_pyramid_adaptor:
            ...
            #
                ...
                kps = self.detect(
                    frame, mask, filter=True
                )  # first, detect by using adaptor on the different pyramid levels
                kps, des = self.compute(
                    frame, kps, filter=False
            ...
            if self.is_detector_equal_to_descriptor:
                ...
                if kVerbose:
                    print(
                        "detector:", self.detector_type.name, ", #features:", len(kps)
                    )
                    print(
                        "descriptor:",
                        self.descriptor_type.name,
        ...
        # filter keypoints
        filter_name = "NONE"
        _, _, bg_mask = self.seg_inference.segment(rgb_frame)
        # bg_mask = np.logical_not(bg_mask)
        bg_mask = cv2.resize(
            bg_mask.astype(np.uint8), (640, 480), interpolation=cv2.INTER_NEAREST
        )
        cv2.imwrite('./bg_mask.png', bg_mask)
        print("kln: ", bg_mask.shape)
        bg_mask = bg_mask == 0
        # bg_mask = np.ones_like(bg_mask)

        filtered_kps = [kp for kp in kps if bg_mask[int(kp.pt[1]), int(kp.pt[0])]]
        des = [
            d
            for i, d in enumerate(des)
            if bg_mask[int(kps[i].pt[1]), int(kps[i].pt[0])]
        ]
        # from IPython import embed; embed()

        kps = np.array(filtered_kps)
        des = np.array(des)
