...
class Slam(object):
    ...
    # @ main track method @
    def track(self, img, frame_id, timestamp=None,mask=None):
        return self.tracking.track(img,frame_id,timestamp,mask=mask)
...
class Tracking(object):
    def __init__(self, system):
        ...
        self.trueX, self.trueY, self.trueZ, self.trueRot = None, None, None, None
    ...
    def track(self, img, frame_id, timestamp=None,mask=None):
        ...
        f_cur = Frame(img, self.camera, mask=mask, timestamp=timestamp)
        self.f_cur = f_cur
        ...
        with self.map.update_lock:
            ...
            self.get_absolute_scale(frame_id)
            self.update_history()
    ...
    def update_history(self):
        ...
        if (self.init_history is True) and (self.trueX is not None):
            ...
            self.R0_est = self.cur_R
            self.R0_gt = self.trueRot
        ...
        if (self.t0_est is not None) and (self.t0_gt is not None):
            ...
            self.trajrot_est.append(np.linalg.inv(self.R0_est) @ self.cur_R)
            self.trajrot_gt.append(np.linalg.inv(self.R0_gt) @ self.trueRot)
    ...
    def get_absolute_scale(self, frame_id):
        if self.groundtruth is not None and kUseGroundTruthScale:
            self.trueX, self.trueY, self.trueZ, self.trueRot, scale = self.groundtruth.getPoseAndAbsoluteScale(frame_id)
            self.trueRot = Rotation.from_quat(self.trueRot).as_matrix()
            return scale
        else:
            self.trueX = 0
            self.trueY = 0
            self.trueZ = 0
            self.trueRot = [1, 1, 1, 0]
            return 1
